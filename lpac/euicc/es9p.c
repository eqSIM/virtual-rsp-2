...
int es9p_authenticate_client_r(struct euicc_ctx *ctx, struct es10b_prepare_download_param *resp,
    ...
    // Try SGP.22 v2.2.2 structure first (separate fields) - osmo-smdpp uses this format
    const char *okey_v22[] = {"profileMetadata", "smdpSigned2", "smdpSignature2", "smdpCertificate", NULL};
    const char oobj_v22[] = {0, 0, 0, 0};
    void **optr_v22[] = {(void **)&resp->b64_profileMetadata, (void **)&resp->b64_smdpSigned2,
                     (void **)&resp->b64_smdpSignature2, (void **)&resp->b64_smdpCertificate, NULL};

    if (es9p_trans_json(ctx, server_address, "/gsma/rsp2/es9plus/authenticateClient", ikey, idata, okey_v22, oobj_v22, optr_v22) == 0) {
        // SGP.22 v2.2.2 - we have the fields directly
    } else {
        // Fallback to SGP.22 v2.5+ structure (prepareDownloadRequest nested)
        cJSON *prepare_download_req = NULL;
        const char *okey_v25[] = {"profileMetadata", "prepareDownloadRequest", NULL};
        const char oobj_v25[] = {0, 1};  // prepareDownloadRequest is an object
        void **optr_v25[] = {(void **)&resp->b64_profileMetadata, (void **)&prepare_download_req, NULL};

        if (es9p_trans_json(ctx, server_address, "/gsma/rsp2/es9plus/authenticateClient", ikey, idata, okey_v25, oobj_v25, optr_v25) == 0) {
            // SGP.22 v2.5+ - extract fields from prepareDownloadRequest object
            if (prepare_download_req && cJSON_IsObject(prepare_download_req)) {
                cJSON *smdpSigned2 = cJSON_GetObjectItem(prepare_download_req, "smdpSigned2");
                cJSON *smdpSignature2 = cJSON_GetObjectItem(prepare_download_req, "smdpSignature2");
                cJSON *smdpCertificate = cJSON_GetObjectItem(prepare_download_req, "smdpCertificate");

                if (smdpSigned2 && cJSON_IsString(smdpSigned2)) {
                    resp->b64_smdpSigned2 = strdup(smdpSigned2->valuestring);
                }
                if (smdpSignature2 && cJSON_IsString(smdpSignature2)) {
                    resp->b64_smdpSignature2 = strdup(smdpSignature2->valuestring);
                }
                if (smdpCertificate && cJSON_IsString(smdpCertificate)) {
                    resp->b64_smdpCertificate = strdup(smdpCertificate->valuestring);
                }

                cJSON_Delete(prepare_download_req);

                if (!resp->b64_smdpSigned2 || !resp->b64_smdpSignature2 || !resp->b64_smdpCertificate) {
                    return -1;
                }
            } else {
                return -1;
