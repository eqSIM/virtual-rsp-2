...
import sys
sys.path.insert(0, '..')
from vrsp_logging import OsmoSmdppLogger as logger, set_log_level, LogLevel
...
class SmDppHttpServer:
    ...
    def initiateAutentication(self, request: IRequest, content: dict) -> dict:
        ...
        # Note: Accept both with and without port (e.g., "host:8443" or "host")
        received_addr = content['smdpAddress'].split(':')[0]  # Strip port if present
        if received_addr != self.server_hostname:
    ...
    def authenticateClient(self, request: IRequest, content: dict) -> dict:
        ...
        try:
            ...
            # Handle case where binary data is misinterpreted as UTF-8
            raise ApiError('8.1', '6.1', f'Invalid data format in AuthenticateServerResponse: {e}')
        except Exception as e:
            raise ApiError('8.1', '6.1', f'Failed to decode AuthenticateServerResponse: {e}')
    ...
    def cancelSession(self, request: IRequest, content: dict) -> dict:
        ...
        if ss.euicc_cert is None:
            raise ApiError('8.10.1', '3.9', 'Session not authenticated - cannot verify cancel signature')
...
def main(argv):
    ...
    # Initialize our colored logging
    set_log_level(LogLevel.DEBUG if args.verbose else LogLevel.INFO)
